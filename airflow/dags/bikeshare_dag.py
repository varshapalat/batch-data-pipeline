...
OPTIONS='--jars /usr/local/target/config-1.3.2.jar --master local'
INGEST_DRIVER = '--class com.thoughtworks.ca.de.batch.ingest_to_data_lake.DailyDriver'
TRANSFORM_DRIVER = '--class com.thoughtworks.ca.de.batch.ingest_to_data_lake.DailyDriver'
APP = '/usr/local/target/tw-pipeline_2.11-0.1.0-SNAPSHOT.jar'
...
def ingest_cleanup_factory(parentDag,config,dateStr,upstreamTask,downstreamTask):
    ...
    ingestSparkTask=SparkSubmitOperator(
        ...
        command=SPARK_SUBMIT+' '+MODE+' '+OPTIONS+' '+INGEST_DRIVER+' '+APP+' '+sourcePath + ' ' +ingestPath ,
        retries=3,
    ...
    transformSparkTask=SparkSubmitOperator(
        ...
        command=SPARK_SUBMIT+' '+MODE+' '+OPTIONS+' '+TRANSFORM_DRIVER+' '+APP+' {{ params.ingestPath }} {{ params.transformPath }}  {{ params.transformDatasetId }}',
        retries=3,
